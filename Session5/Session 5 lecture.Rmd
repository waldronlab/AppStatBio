...
Alternatively as cumulative % variance explained:
```{r, fig.height=3.5}
rafalib::mypar()
plot(cumsum(p$sdev^2)/sum(p$sdev^2)*100, 
  ylab="cumulative % variance explained", ylim=c(0, 100), type="l")
```
...
```{r}
d <- dist(1 - cor(e.standardize.fast))
mds <- cmdscale(d)
```
...
- PCA projection can be applied to new datasets
- PCA is subject to over-fitting
