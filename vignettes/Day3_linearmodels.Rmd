...
1. Create a new variable `leg2` where `L1` / `L2` are merged, and `L3` / `L4` are merged (hint: see `?dplyr::recode_factor`)
2. Create a new variable `leg3` that is an "ordered" factor (hint: you can use `recode_factor` again, or `?factor`)
3. Make "push" the reference level for `spider$type` (hint: see `?relevel`)
...
1. Make histograms of the p-values stored in `p1` and `p2`. Does it look like there are more small p-values than expected if all null hypotheses are true?
...
[html]: https://rpubs.com/lwaldron/AppStatBio2021_day3
