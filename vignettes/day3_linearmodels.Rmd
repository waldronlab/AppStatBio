...
<img src="images/srep01101-f4.jpg" alt="Barplot and Boxplot of spider leg friction coefficients" height="600"/>
...
ggplot(spider, aes(x = interaction(type, leg), y = friction, fill = type)) +
    geom_boxplot(varwidth = TRUE, notch = TRUE, outlier.shape = NA, alpha = 0.8) +
    geom_jitter(width = 0.15, alpha = 0.6, size = 1.4, color = "black") +
...
$$
f(k, \mu) = e^{-\mu} \frac{\mu^k}{k!}
$$

-   where $f$ is the probability of $k$ events (e.g. \# of reads
    counted), and
-   $\mu$ is the mean number of events, so $E[y|x]$
-   $\mu$ is also the variance of the number of events
...
$$
f(k \mid \mu, \alpha) = \frac{\Gamma(k + 1/\alpha)}{k! \, \Gamma(1/\alpha)} 
...
-   where $f$ is still the probability of $k$ events (e.g. \# of reads
    counted),
-   $\mu$ is the mean number of events, so $E[y|x]$
-   $\alpha$ is the dispersion parameter, so $Var(y|x) = \mu + \alpha\mu^2$
    -   larger $\alpha$ means more overdispersion than Poisson
    -   $\alpha \rightarrow 0$: Poisson distribution
...
df_dist <- data.frame(
  Counts = x,
  dist1 = dnbinom(x, size = 10, prob = 0.5),
  dist2 = dnbinom(x, size = 20, prob = 0.5),
  dist3 = dnbinom(x, size = 10, prob = 0.3),
  dist4 = dpois(x, lambda = 9)
)

df_long <- df_dist |>
  pivot_longer(-Counts, names_to = "Distribution", values_to = "Density") |>
  mutate(Distribution = case_match(
    Distribution,
    "dist1" ~ "NB (n=10, p=0.5)",
    "dist2" ~ "NB (n=20, p=0.5)",
    "dist3" ~ "NB (n=10, p=0.3)",
...
ggplot(df_long, aes(x = Counts, y = Density, color = Distribution, linetype = Distribution, shape = Distribution)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
  scale_color_manual(values = c("black", "black", "black", "red")) +
  scale_linetype_manual(values = c("solid", "dashed", "dotted", "solid")) +
  scale_shape_manual(values = c(1, 2, 3, NA)) +
  labs(x = "Counts (k)", y = "Probability density") +
  theme_minimal() +
...
- Work through the [Lab 3: Differential Expression and Double-Dipping](lab3.html) exercises to explore standard differential expression and the dangers of double-dipping.
