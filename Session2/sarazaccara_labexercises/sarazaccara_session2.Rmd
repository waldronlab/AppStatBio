---
title: "sarazaccara_labexcercises"
...
library(dplyr)
x <- dat %>%
  filter(Sex == "M" & Diet == "chow")%>% select(Bodyweight)
...
y <- dat %>%
  filter(Sex == "M" & Diet == "hf")%>% select(Bodyweight)
...
Set the seed at 1. Take a random sample YY of size 25 from y. What is the sample average?

```{r, echo= TRUE}

set.seed(1)
Y <- sample(y_vector, size= 25)
mean(Y)

```

What is the difference in absolute value between y¯−x¯y¯−x¯ and X-Y? 

```{r, echo= TRUE}

abs(mean(y_vector)-mean(x_vector))
abs(mean(X)-mean(Y))

```

Repeat the above for females. Make sure to set the seed to 1 before each sample call. What is the difference in absolute value between y¯−x¯y¯−x¯ and X-Y?

```{r, echo= TRUE, warning=FALSE}

x_female <- dat %>%
  filter(Sex == "F" & Diet == "chow")%>% select(Bodyweight)

xfemale_vector= unlist(x_female)
mean(xfemale_vector)

popsd(xfemale_vector)

y_female <- dat %>%
  filter(Sex == "F" & Diet == "hf")%>% select(Bodyweight)

yfemale_vector= unlist(y_female)
mean(yfemale_vector)

popsd(yfemale_vector)

set.seed(1)
Xfemale <- sample(xfemale_vector, size= 25)
mean(Xfemale)

set.seed(1)
Yfemale <- sample(yfemale_vector, size= 25)
mean(Yfemale)

abs(mean(yfemale_vector)-mean(xfemale_vector))
abs(mean(Xfemale)-mean(Yfemale))
...
## Central Limit Theorem and t-distribution Exercise
...
100*length(which((numbers<=1) & (numbers >=-1)))/length(numbers)
...
numbers <- rnorm(100000)
100*length(which((numbers<=2) & (numbers >=-2)))/length(numbers)
```
...
numbers <- rnorm(100000)
100*length(which((numbers<=3) & (numbers >=-3)))/length(numbers)
...
mean(y)
popsd(y)
100*length(which((y<=(mean(y)+popsd(y))) & (y>=(mean(y)-popsd(y)))))/length(y)
...
100*length(which((y<=(mean(y)+ 2*popsd(y))) & (y>=(mean(y)-2* popsd(y)))))/length(y)

100*length(which((y<=(mean(y)+3*popsd(y))) & (y>=(mean(y)-3*popsd(y)))))/length(y)
...
qqnorm(y)
100*length(which((y<=(mean(y)+3*popsd(y)))))/length(y)
100*length(which((y<=(mean(y)-3*popsd(y)))))/length(y)
...
y_m_hf <- dat %>%
  filter(Sex == "M" & Diet == "hf")%>% select(Bodyweight)
y_m_hf= unlist(y_m_hf)
mean(y_m_hf)
popsd(y_m_hf)
100*length(which((y_m_hf<=(mean(y_m_hf)+popsd(y_m_hf))) & (y_m_hf>=(mean(y_m_hf)-popsd(y_m_hf)))))/length(y_m_hf)

y_f_hf <- dat %>%
  filter(Sex == "F" & Diet == "hf")%>% select(Bodyweight)
y_f_hf= unlist(y_f_hf)
mean(y_f_hf)
popsd(y_f_hf)
100*length(which((y_f_hf<=(mean(y_f_hf)+popsd(y_f_hf))) & (y_f_hf>=(mean(y_f_hf)-popsd(y_f_hf)))))/length(y_f_hf)

y_f_chow <- dat %>%
  filter(Sex == "F" & Diet == "chow")%>% select(Bodyweight)
y_f_chow= unlist(y_f_chow)
mean(y_f_chow)
popsd(y_f_chow)
100*length(which((y_f_chow<=(mean(y_f_chow)+popsd(y_f_chow))) & (y_f_chow>=(mean(y_f_chow)-popsd(y_f_chow)))))/length(y_f_chow)
