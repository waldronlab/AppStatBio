...
```{r}
ppnorm <- 1 - pnorm(abs(tval)) - pnorm(-abs(tval))
```
...
```{r}
print("D) 2*pnorm(-abs(tval))")
print(2*pnorm(-abs(tval)))
```
...
```{r}
df_means <- mean(dat.s) - mean(dat.ns)
std_err <- sd(c(dat.s, dat.ns))/sqrt(length(dat.s)+length(dat.ns))
ci_99_l <- df_means-2.58*std_err
ci_99_h <- df_means+2.58*std_err
print(ci_99_l)
print(ci_99_h)
```
...
```{r}
ci_99_2_l <- df_means-qt(.995, df=2*length(dat.s)-2)*std_err
ci_99_2_h <- df_means+qt(.995, df=2*length(dat.s)-2)*std_err
print(ci_99_2_l)
print(ci_99_2_h)
```
...
```{r}
print(" C) From the original data alone, you can tell whether you have made a Type I error.")
...
```{r}
set.seed(1)
ns_ds <- sample(bwt.nonsmoke, size=5)
set.seed(1)
s_ds <- sample(bwt.smoke, size=5)
t_test <- t.test(ns_ds, s_ds)
...
```{r}
print("C) Find a population for which the null is not true.")
...
```{r}
set.seed(1)
p_vs <- replicate(n=10000, t.test(sample(bwt.nonsmoke, size=5), sample(bwt.smoke, size=5))$p.value < 0.05)
res <- sum(p_vs)/100
print(res)
```
...
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=30), sample(bwt.smoke, size=30))$p.value < 0.05))/100)

print("Sample size: 60")
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=60), sample(bwt.smoke, size=60))$p.value < 0.05))/100)

print("Sample size: 90")
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=90), sample(bwt.smoke, size=90))$p.value < 0.05))/100)

print("Sample size: 120")
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=120), sample(bwt.smoke, size=120))$p.value < 0.05))/100)
```
...
print("Sample size: 30")
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=30), sample(bwt.smoke, size=30))$p.value < 0.01))/100)

print("Sample size: 60")
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=60), sample(bwt.smoke, size=60))$p.value < 0.01))/100)

print("Sample size: 90")
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=90), sample(bwt.smoke, size=90))$p.value < 0.01))/100)

print("Sample size: 120")
set.seed(1)
print(sum(replicate(n=10000, t.test(sample(bwt.nonsmoke, size=120), sample(bwt.smoke, size=120))$p.value < 0.01))/100)
```
